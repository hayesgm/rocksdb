  ...
  public void lockStatusData() throws RocksDBException {
    try (final Options options = new Options().setCreateIfMissing(true);
      ...
      try (final Transaction txn = tdb.beginTransaction(writeOptions)) {
        ...
        final Map<ColumnFamilyHandle, TransactionDB.KeyLockInfo> lockStatus =
            tdb.getLockStatusData();

        assertThat(lockStatus.size()).isEqualTo(1);
        final Set<Map.Entry<ColumnFamilyHandle, TransactionDB.KeyLockInfo>> entrySet = lockStatus.entrySet();
        final Map.Entry<ColumnFamilyHandle, TransactionDB.KeyLockInfo> entry = entrySet.iterator().next();
        final ColumnFamilyHandle columnFamily = entry.getKey();
        assertThat(columnFamily.getName()).isEqualTo("default".getBytes(UTF_8));
