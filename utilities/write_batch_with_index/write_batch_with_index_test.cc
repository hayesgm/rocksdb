...
TEST_P(WriteBatchWithIndexTest, TestComparatorForCF) {
  ...
  batch_->Put(&cf2, "eee", "");
  batch_->Put(&cf1, "ccc", "");
  batch_->Put(&reverse_cf, "a11", "");
  batch_->Put(&cf1, "bbb", "");

  Slice key_slices[] = {"a", "3", "3"};
  Slice value_slice = "";
  batch_->Put(&reverse_cf, SliceParts(key_slices, 3),
              SliceParts(&value_slice, 1));
  batch_->Put(&reverse_cf, "a22", "");
...
TEST_F(WBWIOverwriteTest, TestOverwriteKey) {
  ...
  batch_->Put(&cf2, "aaa", "aaa");
  batch_->Put(&cf2, "eee", "eee");
  batch_->Put(&cf1, "ccc", "");
  batch_->Put(&reverse_cf, "a11", "");
  batch_->Delete(&cf1, "ccc");
  batch_->Put(&reverse_cf, "a33", "a33");
  batch_->Put(&reverse_cf, "a11", "a11");
  Slice slices[] = {"a", "3", "3"};
  batch_->Delete(&reverse_cf, SliceParts(slices, 3));
...
TEST_P(WriteBatchWithIndexTest, TestWBWIIterator) {
  ...
  batch_->Put(&cf2, "a", "a2");
  batch_->Merge(&cf2, "b", "b1");
  batch_->Merge(&cf2, "b", "b2");
  batch_->Delete(&cf2, "d");
  batch_->Merge(&cf2, "d", "d2");
  batch_->Merge(&cf2, "d", "d3");
  batch_->Delete(&cf2, "f");
  AssertIterEqual(iter1.get(), {"a", "c", "e"});
...
TEST_P(WriteBatchWithIndexTest, TestRandomIteraratorWithBase) {
  ...
  for (int rand_seed = 301; rand_seed < 366; rand_seed++) {
    ...
    if (rand_seed % 2 == 0) {
      batch_->Put(&cf2, "zoo", "bar");
    }
    if (rand_seed % 4 == 1) {
      batch_->Put(&cf3, "zoo", "bar");
    }
...
TEST_P(WriteBatchWithIndexTest, TestGetFromBatchMerge) {
  ...
  for (int i = 0; i < 5; i++) {
    batch_->Merge("x", ToString(i));
    expected = expected + "," + ToString(i);

    if (i % 2 == 0) {
      batch_->Put("y", ToString(i / 2));
    }
...
TEST_F(WBWIOverwriteTest, TestGetFromBatchMerge2) {
  ...
  batch_->Put(column_family, "X", "x");
  s = batch_->GetFromBatch(column_family, options_, "X", &value);
  ASSERT_OK(s);
  ASSERT_EQ("x", value);

  batch_->Put(column_family, "X", "x2");
  s = batch_->GetFromBatch(column_family, options_, "X", &value);
  ASSERT_OK(s);
  ASSERT_EQ("x2", value);
...
TEST_P(WriteBatchWithIndexTest, TestGetFromBatchAndDBMerge) {
  ...
  s = db_->Merge(write_opts_, "b", "b1");
  ASSERT_OK(s);

  s = db_->Merge(write_opts_, "c", "c0");
  ASSERT_OK(s);

  s = db_->Merge(write_opts_, "d", "d0");
  ASSERT_OK(s);
  ...
  s = batch_->GetFromBatchAndDB(db_, snapshot_read_options, "e", &value);
  ASSERT_OK(s);
  ASSERT_EQ("e0", value);

  s = db_->Delete(write_opts_, "e");
  ASSERT_OK(s);
...
TEST_F(WBWIOverwriteTest, MutateWhileIteratingCorrectnessTest) {
  for (char c = 'a'; c <= 'z'; ++c) {
    batch_->Put(std::string(1, c), std::string(1, c));
  }
...
TEST_F(WBWIOverwriteTest, MutateWhileIteratingBaseStressTest) {
  ...
  for (int i = 0; i < 1000000; ++i) {
    ...
    switch (random) {
      case 0:
        batch_->Put(std::string(1, c), "xxx");
        break;
      case 1:
        batch_->Put(std::string(2, c), "xxx");
        break;
      case 2:
        batch_->Delete(std::string(1, c));
        break;
      case 3:
        batch_->Delete(std::string(2, c));
        break;
      case 4:
...
TEST_P(WriteBatchWithIndexTest, SingleDeleteTest) {
  ...
  batch_->Put("A", "a3");
  batch_->Delete("B");
  batch_->SingleDelete("B");
  batch_->SingleDelete("C");
...
TEST_P(WriteBatchWithIndexTest, MultiGetTest) {
  ...
  // Write some data to the db for the even numbered keys
  {
    ...
    for (size_t i = 1; i < keys.size(); ++i) {
      ...
      wb.Put(cf0, keys[i], val);
    }
    s = db_->Write(write_opts_, &wb);
    ASSERT_OK(s);
    for (size_t i = 0; i < keys.size(); ++i) {
      std::string value;
      s = db_->Get(read_opts_, keys[i], &value);
    }
  }
...
TEST_F(WBWIKeepTest, GetAfterPut) {
  ...
  batch_->Put("key", "aa");  // Writing aa under key
  ASSERT_OK(batch_->GetFromBatch(cf0, options_, "key", &value));
  ASSERT_EQ(value, "aa");
  ASSERT_OK(batch_->GetFromBatchAndDB(db_, read_opts_, "key", &value));
  ASSERT_EQ(value, "aa");

  batch_->Merge("key", "bb");  // Merging bb under key
  ASSERT_OK(batch_->GetFromBatch(cf0, options_, "key", &value));
  ASSERT_EQ(value, "aa,bb");
  ASSERT_OK(batch_->GetFromBatchAndDB(db_, read_opts_, "key", &value));
  ASSERT_EQ(value, "aa,bb");

  batch_->Merge("key", "cc");  // Merging cc under key
  ASSERT_OK(batch_->GetFromBatch(cf0, options_, "key", &value));
...
TEST_P(WriteBatchWithIndexTest, GetAfterMergePut) {
  ...
  batch_->Merge("key", "aa");  // Merging aa under key
  ...
  batch_->Put("key", "cc");  // Writing cc under key
  ASSERT_OK(batch_->GetFromBatch(cf0, options_, "key", &value));
  ASSERT_EQ(value, "cc");
  ASSERT_OK(batch_->GetFromBatchAndDB(db_, read_opts_, "key", &value));
  ASSERT_EQ(value, "cc");

  batch_->Merge("key", "dd");  // Merging dd under key
  ASSERT_OK(batch_->GetFromBatch(cf0, options_, "key", &value));
...
TEST_P(WriteBatchWithIndexTest, GetAfterMergeDelete) {
  ...
  batch_->Delete("key");  // Delete key from batch
  s = batch_->GetFromBatch(cf0, options_, "key", &value);
  ASSERT_TRUE(s.IsNotFound());
